...
7. **How this report was made** — opens with the skill link line from `seo-report`, pointing at `https://github.com/Arofrancisco/open-seo/blob/main/.agents/skills/local-seo/SKILL.md` ("PlanetaSEO Local SEO skill"), then which tools returned what, plus a note reading each missing grid rank against that point's `resultsCount` rather than calling it invisibility.
